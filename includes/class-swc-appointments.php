...
final class SWC_Appointments {
	...
	public function submit() {
		$this->require_login( __( 'Log in to request an appointment.', 'worldwide-clinic' ) );
		check_admin_referer( 'swc_submit_appointment', 'swc_nonce' );

		$user_id = get_current_user_id();
		if ( SWC_Helpers::rate_limit_hit( $user_id, 5, HOUR_IN_SECONDS ) ) {
			wp_die( esc_html__( 'Please wait before submitting another request.', 'worldwide-clinic' ), '', array( 'response' => 429 ) );
		}

		$doctor = isset( $_POST['doctor_id'] ) ? absint( $_POST['doctor_id'] ) : 0;
		if ( ! in_array( $doctor, SWC_Helpers::requestable_doctor_ids(), true ) ) {
			wp_die( esc_html__( 'Choose an eligible verified doctor.', 'worldwide-clinic' ), '', array( 'response' => 400 ) );
		}
		...
		if ( ! in_array( $mode, array( 'online', 'in-person' ), true ) || ! SWC_Helpers::doctor_accepts_mode( $doctor, $mode ) ) {
			wp_die( esc_html__( 'Choose a consultation type enabled by the selected doctor.', 'worldwide-clinic' ), '', array( 'response' => 400 ) );
		}
		...
		if ( ! $utc || strtotime( $utc . ' UTC' ) <= time() ) {
			wp_die( esc_html__( 'Choose a valid, unambiguous future date and time.', 'worldwide-clinic' ), '', array( 'response' => 400 ) );
		}
		...
		if ( ! SWC_Helpers::slot_is_available( $doctor, $utc, $duration ) ) {
			wp_die( esc_html__( 'The requested time is outside the doctor’s published schedule or conflicts with an accepted appointment.', 'worldwide-clinic' ), '', array( 'response' => 409 ) );
		}
		...
		if ( ! $phone || ! $whatsapp || '' === trim( $country ) || '' === trim( $reason ) || empty( $_POST['consent'] ) || empty( $_POST['emergency_confirm'] ) ) {
			wp_die( esc_html__( 'Complete the required contact, location, reason, emergency acknowledgment, and consent fields.', 'worldwide-clinic' ), '', array( 'response' => 400 ) );
		}
		...
		$id = wp_insert_post(
			array(
				...
				'post_status' => 'private',
				'post_title'  => sprintf( __( 'Appointment Request — %s', 'worldwide-clinic' ), current_time( 'Y-m-d H:i' ) ),
				'post_author' => $user_id,
			),
			true
		);
		if ( is_wp_error( $id ) ) {
			wp_die( esc_html__( 'The request could not be saved.', 'worldwide-clinic' ), '', array( 'response' => 500 ) );
		}
		...
		if ( ! SWC_Helpers::audit(
			...
			wp_die( esc_html__( 'The request could not be safely audited. Nothing was submitted.', 'worldwide-clinic' ), '', array( 'response' => 500 ) );
		}

		$this->notify_participants( $id, 'appointment-requested', __( 'New appointment request', 'worldwide-clinic' ), __( 'A new appointment request was received.', 'worldwide-clinic' ), 'normal' );
		$this->redirect( 'patient', array( 'requested' => '1' ) );
	}

	public function patient_cancel() {
		$this->require_login( __( 'Access denied.', 'worldwide-clinic' ) );
		...
		if ( ! SWC_Helpers::can_patient_manage( $id ) ) {
			wp_die( esc_html__( 'You cannot manage this appointment.', 'worldwide-clinic' ), '', array( 'response' => 403 ) );
		}
		$this->process_locked_transition( $id, 'patient', 'cancelled', 'patient-cancelled', __( 'Cancelled by patient', 'worldwide-clinic' ) );
		$this->notify_participants( $id, 'appointment-cancelled', __( 'Appointment cancelled', 'worldwide-clinic' ), __( 'The appointment request was cancelled.', 'worldwide-clinic' ), 'high' );
	...
	public function patient_accept_reschedule() {
		...
		$result = SWC_Helpers::with_lock(
			...
			function () use ( $id, $expected_status, $expected_version ) {
				...
				if ( ! SWC_Helpers::can_transition( 'patient', $current, 'accepted' ) ) {
					return new WP_Error( 'swc_transition', __( 'This reschedule can no longer be accepted.', 'worldwide-clinic' ) );
				}
				...
				if ( ! $proposed || strtotime( $proposed . ' UTC' ) <= time() || ( $expires && strtotime( $expires . ' UTC' ) <= time() ) ) {
					return new WP_Error( 'swc_expired', __( 'The proposed time has expired.', 'worldwide-clinic' ) );
				}
				if ( ! SWC_Helpers::slot_is_available( $doctor, $proposed, $duration, $id ) ) {
					return new WP_Error( 'swc_conflict', __( 'The proposed time is no longer available.', 'worldwide-clinic' ) );
				}
				update_post_meta( $id, '_swc_preferred_at_utc', $proposed );
				update_post_meta( $id, '_swc_status', 'accepted' );
				...
				if ( ! SWC_Helpers::audit( $id, 'reschedule-accepted', array( 'old_status' => $current, 'new_status' => 'accepted', 'reason' => __( 'Accepted by patient', 'worldwide-clinic' ) ) ) ) {
					return new WP_Error( 'swc_audit', __( 'The update could not be audited.', 'worldwide-clinic' ) );
				}
				return true;
			}
		);
		$this->die_on_error( $result );
		$this->notify_participants( $id, 'reschedule-accepted', __( 'Proposed time accepted', 'worldwide-clinic' ), __( 'The patient accepted the proposed appointment time.', 'worldwide-clinic' ), 'high' );
		$this->redirect( 'patient' );
	}
		...
		$result = SWC_Helpers::with_lock(
			...
			function () use ( $id, $accept, $expected_version ) {
				if ( SWC_Helpers::record_version( $id ) !== $expected_version ) {
					return new WP_Error( 'swc_stale', __( 'This appointment changed. Refresh before responding.', 'worldwide-clinic' ) );
				}
				...
				if ( ! $new_doctor || ( $expires && strtotime( $expires . ' UTC' ) <= time() ) ) {
					return new WP_Error( 'swc_expired', __( 'The reassignment proposal has expired.', 'worldwide-clinic' ) );
				}
				$old_doctor = absint( SWC_Helpers::meta( $id, 'doctor_id' ) );
				if ( $accept ) {
					if ( ! in_array( $new_doctor, SWC_Helpers::doctor_ids(), true ) ) {
						return new WP_Error( 'swc_doctor', __( 'The proposed doctor is no longer eligible.', 'worldwide-clinic' ) );
					}
					$mode = (string) SWC_Helpers::meta( $id, 'consultation_type' );
					if ( ! SWC_Helpers::doctor_accepts_mode( $new_doctor, $mode ) ) {
						return new WP_Error( 'swc_mode', __( 'The proposed doctor does not offer the selected consultation type.', 'worldwide-clinic' ) );
					}
					...
					if ( ! SWC_Helpers::slot_is_available( $new_doctor, $utc, $duration, $id ) ) {
						return new WP_Error( 'swc_slot', __( 'The current time is not available with the proposed doctor.', 'worldwide-clinic' ) );
					}
				...
				if ( ! SWC_Helpers::audit(
					...
					array(
						...
						'reason'        => $accept ? __( 'Accepted by patient', 'worldwide-clinic' ) : __( 'Declined by patient', 'worldwide-clinic' ),
					)
				) ) {
					return new WP_Error( 'swc_audit', __( 'The response could not be audited.', 'worldwide-clinic' ) );
				}
				return array( 'old_doctor' => $old_doctor, 'new_doctor' => $new_doctor );
			}
		);
		$this->die_on_error( $result );
		$title = $accept ? __( 'Doctor reassignment accepted', 'worldwide-clinic' ) : __( 'Doctor reassignment declined', 'worldwide-clinic' );
		$body  = $accept ? __( 'The patient accepted the proposed doctor reassignment.', 'worldwide-clinic' ) : __( 'The patient declined the proposed doctor reassignment.', 'worldwide-clinic' );
	...
	public function doctor_update() {
		...
		$result = SWC_Helpers::with_lock(
			...
			function () use ( $id, $next, $expected_status, $expected_version, $private_note, $patient_message ) {
				...
				if ( ! isset( SWC_Helpers::statuses()[ $next ] ) ) {
					return new WP_Error( 'swc_status', __( 'Invalid appointment status.', 'worldwide-clinic' ) );
				}
				$status_changed = $next !== $current;
				if ( $status_changed && ! SWC_Helpers::can_transition( 'doctor', $current, $next ) ) {
					return new WP_Error( 'swc_transition', __( 'That status transition is not permitted.', 'worldwide-clinic' ) );
				}
				...
				if ( 'accepted' === $next && $status_changed ) {
					...
					if ( ! SWC_Helpers::slot_is_available( $doctor, $utc, $duration, $id ) ) {
						return new WP_Error( 'swc_conflict', __( 'This time is outside your published schedule or conflicts with another accepted appointment.', 'worldwide-clinic' ) );
					}
				}
		...
		$this->notify_participants(
			...
			__( 'Appointment updated', 'worldwide-clinic' ),
			$result['status_changed'] ? sprintf( __( 'The appointment status is now %s.', 'worldwide-clinic' ), $label ) : __( 'The doctor updated the appointment message.', 'worldwide-clinic' ),
	...
	public function save_availability() {
		$this->require_login( __( 'Verified doctor access is required.', 'worldwide-clinic' ) );
		$id = get_current_user_id();
		if ( ! SWC_Helpers::is_verified_doctor( $id ) ) {
			wp_die( esc_html__( 'Verified doctor access is required.', 'worldwide-clinic' ), '', array( 'response' => 403 ) );
		}
		...
		if ( ! SWC_Helpers::availability_is_valid( $data ) ) {
			wp_die( esc_html__( 'Publish at least one day, a valid start/end window, a valid time zone, and one consultation type. Accepting and unavailable cannot both be enabled.', 'worldwide-clinic' ), '', array( 'response' => 400 ) );
		}
