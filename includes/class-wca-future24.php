...
final class WCA_Future24 {
		...
		foreach ( self::waitlist_candidates( $clinic_id ) as $wait ) {
			...
			try {
				$outcome = WCA_Repository::transaction( function () use ( $table, $wait, $start, $end, $clinic_id, $service_ref, $recipient_id ) {
					...
					if ( is_wp_error( $offer ) ) { return $offer; }
				...
				}, 'wca_waitlist_offer_transaction' );
	...
	private static function clinic_id_from_public_ref( $ref ) {
...
self::release_semantic_lock( $lock );
	...
	public static function safe_reschedule( $appointment_ref, $data, $actor = 0 ) {
		...
		$audit = self::audit( 'F08-FUT-06', 'safe_reschedule_completed', $appointment_ref, array( 'compensation_safe' => true ), $actor, false );
	...
	public static function questionnaire_for_appointment( $appointment_ref, $actor = 0 ) {
		...
		$service_ref = $service && ! empty( $service['public_ref'] ) ? strtolower( (string) $service['public_ref'] ) : '';
		$table = self::tables()['records'];
		$rows = self::feature_rows_for_clinic( 'F08-FUT-11', $clinic_id, 'template_active', 'questionnaire' );
	...
	private static function prerequisite_state( $appointment_id ) {
		...
		$service_ref = $service && ! empty( $service['public_ref'] ) ? strtolower( (string) $service['public_ref'] ) : '';
		$rules = self::feature_rows_for_clinic( 'F08-FUT-13', $clinic_id, 'rule_active', 'payload' );
	...
	public static function arrive( $appointment_ref, $actor = 0 ) {
		...
		$lock = self::semantic_lock( 'arrival', $id );
		if ( is_wp_error( $lock ) ) { return $lock; }
		try {
			$table = self::tables()['records'];
			$existing = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table} WHERE feature_id='F08-FUT-15' AND appointment_id=%d AND status='arrived' AND (expires_at IS NULL OR expires_at>%s) ORDER BY id DESC LIMIT 1", $id, WCA_Repository::now() ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public static function queue_position( $appointment_ref, $actor = 0 ) {
		...
		$ahead_raw = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(DISTINCT appointment_id) FROM {$table} WHERE feature_id='F08-FUT-15' AND clinic_id=%d AND status='arrived' AND created_at<%s AND (expires_at IS NULL OR expires_at>%s)", $clinic_id, $current['created_at'], $now ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public static function revoke_participant( $appointment_ref, $participant_ref, $actor = 0 ) {
		...
		if ( is_wp_error( $lock ) ) { return $lock; }
		try {
			return WCA_Repository::transaction( function () use ( $participant_ref, $id, $appointment_ref, $actor_id ) {
				global $wpdb;
				...
				if ( 1 !== (int) $updated ) { return new WP_Error( 'wca_support_revoke_conflict', __( 'The support participant changed. Refresh and try again.', 'worldwide-clinic-appointments' ), array( 'status' => 409 ) ); }
				$payload = json_decode( (string) $row['payload_json'], true );
				$payload = is_array( $payload ) ? $payload : array();
			...
			}, 'wca_support_revoke_transaction' );
		} finally {
			self::release_semantic_lock( $lock );
		}
	}
	...
	public static function smart_find( $params ) {
		...
		foreach(self::apply_slot_policies((array)($result['slots'] ?? array())) as $slot){$external=self::external_busy_conflict($slot['practitioner_ref'] ?? '',$slot['start_utc'] ?? '',$slot['end_utc'] ?? ''); if(is_wp_error($external)){return $external;} if(!$external){$items[]=$slot;}}
