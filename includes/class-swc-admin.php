...
final class SWC_Admin {
	...
	public function menu() {
		add_menu_page(
			__( 'Clinic Management', 'worldwide-clinic' ),
			__( 'Clinic Management', 'worldwide-clinic' ),
		...
		add_submenu_page( 'clinic-management', __( 'Appointments', 'worldwide-clinic' ), __( 'Appointments', 'worldwide-clinic' ), 'manage_worldwide_clinic', 'clinic-management', array( $this, 'page' ) );
		add_submenu_page( 'clinic-management', __( 'Clinic Settings', 'worldwide-clinic' ), __( 'Clinic Settings', 'worldwide-clinic' ), 'manage_worldwide_clinic', 'clinic-settings', array( $this, 'settings' ) );
		add_submenu_page( 'clinic-management', __( 'System Check', 'worldwide-clinic' ), __( 'System Check', 'worldwide-clinic' ), 'manage_worldwide_clinic', 'clinic-system-check', array( $this, 'system_check' ) );
	}
	...
	public function page() {
		...
		<div class="wrap swc-admin">
			<h1><?php esc_html_e( 'Clinic Management', 'worldwide-clinic' ); ?></h1>
			<p><?php esc_html_e( 'Private scheduling information is limited to the patient, the assigned verified doctor, and authorized clinic administrators.', 'worldwide-clinic' ); ?></p>
			<form class="swc-admin-filter" method="get">
				<input type="hidden" name="page" value="clinic-management">
				<label class="screen-reader-text" for="swc-filter-status"><?php esc_html_e( 'Filter by status', 'worldwide-clinic' ); ?></label>
				<select id="swc-filter-status" name="status">
					<option value=""><?php esc_html_e( 'All statuses', 'worldwide-clinic' ); ?></option>
					<?php foreach ( SWC_Helpers::statuses() as $key => $label ) : ?>
						<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $status, $key ); ?>><?php echo esc_html( $label ); ?></option>
					<?php endforeach; ?>
				</select>
				<label class="screen-reader-text" for="swc-filter-doctor"><?php esc_html_e( 'Filter by doctor', 'worldwide-clinic' ); ?></label>
				<select id="swc-filter-doctor" name="doctor">
					<option value="0"><?php esc_html_e( 'All doctors', 'worldwide-clinic' ); ?></option>
				...
				<button class="button"><?php esc_html_e( 'Filter', 'worldwide-clinic' ); ?></button>
			</form>
			<table class="widefat striped">
				<thead><tr><th><?php esc_html_e( 'Reference', 'worldwide-clinic' ); ?></th><th><?php esc_html_e( 'Patient', 'worldwide-clinic' ); ?></th><th><?php esc_html_e( 'Doctor', 'worldwide-clinic' ); ?></th><th><?php esc_html_e( 'Requested time', 'worldwide-clinic' ); ?></th><th><?php esc_html_e( 'Status', 'worldwide-clinic' ); ?></th><th><?php esc_html_e( 'Management', 'worldwide-clinic' ); ?></th></tr></thead>
				<tbody>
				<?php if ( $query->have_posts() ) : ?>
					...
					<tr>
						...
						<td><?php echo esc_html( SWC_Helpers::statuses()[ $current ] ); ?><br><small><?php echo esc_html( sprintf( __( 'Version %d', 'worldwide-clinic' ), $version ) ); ?></small></td>
						<td>
				...
				<?php else : ?>
					<tr><td colspan="6"><?php esc_html_e( 'No appointments found.', 'worldwide-clinic' ); ?></td></tr>
				<?php endif; ?>
				</tbody>
			</table>
			<?php
	...
	private function management_form( $id, $current, $version, $doctors ) {
		...
		<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
			...
			<label><span><?php esc_html_e( 'Assigned doctor', 'worldwide-clinic' ); ?></span>
			...
			<label><span><?php esc_html_e( 'Status', 'worldwide-clinic' ); ?></span>
				<select name="status">
			...
			<div class="swc-admin-reschedule">
				<label><span><?php esc_html_e( 'Proposed date', 'worldwide-clinic' ); ?></span><input type="date" name="new_date"></label>
				<label><span><?php esc_html_e( 'Proposed time', 'worldwide-clinic' ); ?></span><input type="time" name="new_time"></label>
				<label><span><?php esc_html_e( 'Time zone', 'worldwide-clinic' ); ?></span><select name="new_timezone"><?php foreach ( SWC_Helpers::timezones() as $timezone ) : ?><option value="<?php echo esc_attr( $timezone ); ?>" <?php selected( 'UTC', $timezone ); ?>><?php echo esc_html( $timezone ); ?></option><?php endforeach; ?></select></label>
			</div>
			<label><span><?php esc_html_e( 'Required internal reason', 'worldwide-clinic' ); ?></span><textarea name="note" maxlength="1000" required></textarea></label>
			<p class="description"><?php esc_html_e( 'Changing the doctor creates a patient-consent proposal; it does not immediately disclose the request to the proposed doctor.', 'worldwide-clinic' ); ?></p>
			<button class="button button-primary"><?php esc_html_e( 'Save', 'worldwide-clinic' ); ?></button>
		</form>
		<?php
	}
	...
	private function audit_timeline( $id ) {
		...
		?>
		<details class="swc-audit"><summary><?php esc_html_e( 'Audit history', 'worldwide-clinic' ); ?></summary>
			<?php if ( $rows ) : ?><ol>
				...
				<?php endforeach; ?>
			</ol><?php else : ?><p><?php esc_html_e( 'No audit entries were found.', 'worldwide-clinic' ); ?></p><?php endif; ?>
		</details>
		<?php
	}
	...
	public function update() {
		...
		if ( SWC_Helpers::TYPE !== get_post_type( $id ) ) {
			wp_die( esc_html__( 'Invalid appointment.', 'worldwide-clinic' ), '', array( 'response' => 400 ) );
		}
		...
		if ( '' === trim( $note ) || ! isset( SWC_Helpers::statuses()[ $next ] ) || ! in_array( $doctor, SWC_Helpers::doctor_ids(), true ) ) {
			wp_die( esc_html__( 'Provide a valid doctor, status, and internal reason.', 'worldwide-clinic' ), '', array( 'response' => 400 ) );
		}
		...
		$result = SWC_Helpers::with_lock(
			...
			function () use ( $id, $next, $doctor, $expected_status, $expected_version, $note ) {
				...
				if ( $doctor_changed ) {
					if ( $status_changed ) {
						return new WP_Error( 'swc_combined', __( 'Propose reassignment separately from a status change.', 'worldwide-clinic' ) );
					}
					...
					if ( ! SWC_Helpers::audit( $id, 'reassignment-proposed', array( 'old_doctor_id' => $current_doctor, 'new_doctor_id' => $doctor, 'reason' => $note ) ) ) {
						return new WP_Error( 'swc_audit', __( 'The reassignment proposal could not be audited.', 'worldwide-clinic' ) );
					}
				...
				if ( $status_changed && ! SWC_Helpers::can_transition( 'admin', $current, $next ) ) {
					return new WP_Error( 'swc_transition', __( 'That status transition is not permitted. Terminal appointments cannot be revived.', 'worldwide-clinic' ) );
				}
				$duration = absint( SWC_Helpers::meta( $id, 'appointment_duration', 30 ) );
				$details  = array();
				if ( 'accepted' === $next && $status_changed ) {
					$utc = (string) SWC_Helpers::meta( $id, 'preferred_at_utc' );
					if ( ! SWC_Helpers::slot_is_available( $current_doctor, $utc, $duration, $id ) ) {
						return new WP_Error( 'swc_conflict', __( 'The appointment time is no longer available.', 'worldwide-clinic' ) );
					}
				}
				if ( 'completed' === $next && $status_changed ) {
					$utc = (string) SWC_Helpers::meta( $id, 'preferred_at_utc' );
					if ( ! $utc || strtotime( $utc . ' UTC' ) > time() ) {
						return new WP_Error( 'swc_future', __( 'A future appointment cannot be marked completed.', 'worldwide-clinic' ) );
					}
				}
				if ( 'reschedule-requested' === $next ) {
					...
					if ( ! $new || strtotime( $new . ' UTC' ) <= time() || ! SWC_Helpers::slot_is_available( $current_doctor, $new, $duration, $id ) ) {
						return new WP_Error( 'swc_reschedule', __( 'Provide a future, unambiguous, available reschedule time.', 'worldwide-clinic' ) );
					}
					$expiry = min( strtotime( '+48 hours' ), strtotime( $new . ' UTC' ) - HOUR_IN_SECONDS );
					if ( $expiry <= time() ) {
						return new WP_Error( 'swc_expiry', __( 'The proposed time is too soon for patient confirmation.', 'worldwide-clinic' ) );
					}
				...
				if ( ! SWC_Helpers::audit( $id, $event, array( 'old_status' => $current, 'new_status' => $next, 'old_doctor_id' => $current_doctor, 'new_doctor_id' => $current_doctor, 'reason' => $note, 'details' => $details ) ) ) {
					return new WP_Error( 'swc_audit', __( 'The administrator update could not be audited.', 'worldwide-clinic' ) );
				}
		...
		if ( 'reassignment-proposed' === $result['event'] ) {
			SWC_Helpers::notify_user( $patient, 'reassignment-proposed', __( 'Doctor reassignment requires your consent', 'worldwide-clinic' ), __( 'An administrator proposed a different doctor. Review and accept or decline the proposal in My Appointments.', 'worldwide-clinic' ), $id, ! empty( $pages['patient'] ) ? get_permalink( $pages['patient'] ) : home_url( '/' ), 'high' );
			SWC_Helpers::notify_user( absint( $result['old_doctor'] ), 'reassignment-proposed', __( 'Doctor reassignment proposed', 'worldwide-clinic' ), __( 'An administrator proposed reassignment. The current assignment remains unchanged until the patient consents.', 'worldwide-clinic' ), $id, ! empty( $pages['doctor'] ) ? get_permalink( $pages['doctor'] ) : home_url( '/' ), 'normal' );
		} else {
			SWC_Helpers::notify_user( $patient, 'appointment-admin-updated', __( 'Appointment updated', 'worldwide-clinic' ), __( 'An authorized clinic administrator updated your appointment.', 'worldwide-clinic' ), $id, ! empty( $pages['patient'] ) ? get_permalink( $pages['patient'] ) : home_url( '/' ), 'high' );
			SWC_Helpers::notify_user( absint( $result['new_doctor'] ), 'appointment-admin-updated', __( 'Appointment updated', 'worldwide-clinic' ), __( 'An authorized clinic administrator updated an assigned appointment.', 'worldwide-clinic' ), $id, ! empty( $pages['doctor'] ) ? get_permalink( $pages['doctor'] ) : home_url( '/' ), 'high' );
		}
	...
	public function settings() {
		$this->guard();
		?>
		<div class="wrap swc-admin"><h1><?php esc_html_e( 'Clinic Settings', 'worldwide-clinic' ); ?></h1>
		<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
			<input type="hidden" name="action" value="swc_save_clinic_settings"><?php wp_nonce_field( 'swc_save_clinic_settings' ); ?>
			<table class="form-table"><tr><th><label for="swc_phone"><?php esc_html_e( 'Clinic phone', 'worldwide-clinic' ); ?></label></th><td><input id="swc_phone" name="clinic_phone" maxlength="18" value="<?php echo esc_attr( get_option( 'swc_clinic_phone', '' ) ); ?>" placeholder="+12025550123"></td></tr>
			<tr><th><label for="swc_whatsapp"><?php esc_html_e( 'Clinic WhatsApp', 'worldwide-clinic' ); ?></label></th><td><input id="swc_whatsapp" name="clinic_whatsapp" maxlength="18" value="<?php echo esc_attr( get_option( 'swc_clinic_whatsapp', '' ) ); ?>" placeholder="+12025550123"></td></tr>
			<tr><th><label for="swc_emergency"><?php esc_html_e( 'Emergency notice', 'worldwide-clinic' ); ?></label></th><td><textarea id="swc_emergency" name="emergency_notice" rows="4" maxlength="1000" class="large-text"><?php echo esc_textarea( SWC_Helpers::emergency_notice() ); ?></textarea></td></tr></table>
			<button class="button button-primary"><?php esc_html_e( 'Save Clinic Settings', 'worldwide-clinic' ); ?></button>
		</form></div>
		<?php
	}
	...
	public function save_settings() {
		...
		if ( ( isset( $_POST['clinic_phone'] ) && '' !== trim( (string) $_POST['clinic_phone'] ) && ! $phone ) || ( isset( $_POST['clinic_whatsapp'] ) && '' !== trim( (string) $_POST['clinic_whatsapp'] ) && ! $whatsapp ) || '' === trim( $emergency ) ) {
			wp_die( esc_html__( 'Provide valid clinic contact numbers and a nonempty emergency notice.', 'worldwide-clinic' ), '', array( 'response' => 400 ) );
		}
	...
	public function system_check() {
		...
		?>
		<div class="wrap swc-admin"><h1><?php esc_html_e( 'File 08 System Check', 'worldwide-clinic' ); ?></h1>
		...
		<?php if ( get_option( 'swc_last_audit_error' ) ) : ?><p class="notice notice-error inline"><?php echo esc_html( sprintf( __( 'Last audit error: %s', 'worldwide-clinic' ), get_option( 'swc_last_audit_error' ) ) ); ?></p><?php endif; ?>
		<?php if ( get_option( 'swc_last_delivery_error' ) ) : ?><p class="notice notice-warning inline"><?php esc_html_e( 'A notification fallback recently failed. Review File 19 and transactional email configuration.', 'worldwide-clinic' ); ?></p><?php endif; ?>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><?php wp_nonce_field( 'swc_complete_repair' ); ?><input type="hidden" name="action" value="swc_complete_repair"><button class="button button-primary"><?php esc_html_e( 'Run Complete Repair', 'worldwide-clinic' ); ?></button></form>
		<hr><h2><?php esc_html_e( 'Irreversible Data Purge', 'worldwide-clinic' ); ?></h2><p><?php esc_html_e( 'Create and verify a full backup first. Purge deletes every File 08 appointment, audit entry, rate-limit record, option, and user availability field.', 'worldwide-clinic' ); ?></p>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><?php wp_nonce_field( 'swc_purge_data' ); ?><input type="hidden" name="action" value="swc_purge_data"><label><?php esc_html_e( 'Type PURGE FILE 08 to confirm', 'worldwide-clinic' ); ?><input name="confirmation" autocomplete="off" required></label> <button class="button button-secondary"><?php esc_html_e( 'Purge File 08 Data', 'worldwide-clinic' ); ?></button></form></div>
		<?php
	}
	...
	public function purge() {
		...
		if ( 'PURGE FILE 08' !== $confirmation ) {
			wp_die( esc_html__( 'The confirmation phrase did not match. No data was deleted.', 'worldwide-clinic' ), '', array( 'response' => 400 ) );
		}
	...
	private function guard() {
		if ( ! current_user_can( 'manage_worldwide_clinic' ) ) {
			wp_die( esc_html__( 'You are not allowed to manage the clinic.', 'worldwide-clinic' ), '', array( 'response' => 403 ) );
		}
	}
	...
	public function notice() {
		if ( current_user_can( 'activate_plugins' ) && get_transient( 'swc_activation_notice' ) ) {
			...
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Worldwide Clinic is active. Review System Check, Clinic Settings, and doctor availability before staging acceptance.', 'worldwide-clinic' ) . '</p></div>';
		}
	}
}
