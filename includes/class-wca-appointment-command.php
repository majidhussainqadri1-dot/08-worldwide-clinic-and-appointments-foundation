...
final class WCA_Appointment_Command {
	...
	public static function request( $data, $actor_user_id = 0 ) {
		...
		if ( empty( $data['emergency_acknowledged'] ) ) {
		...
		if ( $remote && empty( $data['telehealth_consent'] ) ) {
		...
		if ( $remote && ! empty( $result['appointment_id'] ) ) {
			self::ensure_context_consent( absint( $result['appointment_id'] ), 'teleconsult', $actor_user_id );
		}
		if ( ! empty( $result['appointment_id'] ) ) {
			self::ensure_context_consent( absint( $result['appointment_id'] ), 'privacy_notice', $actor_user_id );
		}
		return $result;
	}
