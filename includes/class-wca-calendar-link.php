 ...
 *
 * A normal browser navigation cannot attach a REST nonce reliably. This
 * module therefore issues a short-lived bearer signature bound to the opaque
 * appointment reference and current participant. The exported ICS contains
 * scheduling facts only and no reason, intake, complaint, patient name or
 * other clinical narrative.
 *
...
final class WCA_Calendar_Link {
	...
	public static function boot() {
		add_action( 'rest_api_init', array( __CLASS__, 'register_route' ), 70 );
	}

	public static function register_route() {
		register_rest_route(
			'wca/v1',
			'/calendar-links/(?P<ref>[0-9a-fA-F-]{36})\.ics',
			array(
	...
	public static function url( $appointment_ref, $user_id = 0 ) {
		...
		if ( ! $user_id || ! preg_match( '/^[0-9a-f-]{36}$/', $appointment_ref ) ) {
			return '';
		}
		$id = self::appointment_id( $appointment_ref );
		if ( ! $id || is_wp_error( WCA_Authorization::can_view_appointment( $id, $user_id ) ) ) {
			return '';
		}
		$exp = time() + self::TTL;
		...
		return add_query_arg(
			array(
				'sub' => rawurlencode( $subject ),
				'exp' => $exp,
				'sig' => rawurlencode( $sig ),
			),
			rest_url( 'wca/v1/calendar-links/' . rawurlencode( $appointment_ref ) . '.ics' )
		);
	}
	...
	public static function download( WP_REST_Request $request ) {
		...
		$access = WCA_Authorization::can_view_appointment( $id, $user_id );
		if ( is_wp_error( $access ) ) {
			return new WP_Error( 'wca_calendar_link_invalid', __( 'This calendar link is invalid or expired.', 'worldwide-clinic-appointments' ), array( 'status' => 403 ) );
		}
		$status = SWC_Helpers::status( $id );
		if ( ! in_array( $status, array( 'confirmed', 'reschedule_pending', 'checked_in', 'completed' ), true ) ) {
			return new WP_Error( 'wca_calendar_state', __( 'Calendar export is unavailable for this appointment state.', 'worldwide-clinic-appointments' ), array( 'status' => 409 ) );
		}
		$start = (string) SWC_Helpers::meta( $id, 'preferred_at_utc', '' );
		$end   = (string) SWC_Helpers::meta( $id, 'appointment_end_utc', '' );
		if ( ! $start || ! $end ) {
			return new WP_Error( 'wca_calendar_time', __( 'Confirmed appointment time is unavailable.', 'worldwide-clinic-appointments' ), array( 'status' => 409 ) );
		}
		$uid = strtolower( $ref ) . '@' . sanitize_key( wp_parse_url( home_url( '/' ), PHP_URL_HOST ) );
	...
	private static function appointment_id( $ref ) {
		$ids = get_posts( array(
			...
			'meta_value'     => strtolower( sanitize_text_field( $ref ) ),
		) );
...
function wca_signed_calendar_url( $appointment_ref, $user_id = 0 ) {
	return WCA_Calendar_Link::url( $appointment_ref, $user_id );
}
