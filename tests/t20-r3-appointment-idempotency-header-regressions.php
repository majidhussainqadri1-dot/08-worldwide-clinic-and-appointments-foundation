...
if ( is_string( $source ) ) {
    ...
    t20r3_check( 'REST boundary delegates normalized command data', false !== strpos( $source, 'self::request( $data, $user_id )' ) );
    t20r3_check( 'domain service receives normalized command data', false !== strpos( $source, 'WCA_Service::request_appointment( $data, $actor_user_id )' ) );
}
