...
$failures = array();
$checks = 0;
function f08pkg_file( $root, $path ) {
	global $failures;
	$file = $root . '/' . $path;
...
function f08pkg_has( $label, $source, $needle ) {
	global $failures, $checks;
	$checks++;
	if ( false === strpos( $source, $needle ) ) { $failures[] = $label . ' missing: ' . $needle; }
}
function f08pkg_lacks( $label, $source, $needle ) {
	global $failures, $checks;
	$checks++;
	if ( false !== strpos( $source, $needle ) ) { $failures[] = $label . ' forbidden: ' . $needle; }
}
$plugin = f08pkg_file( $root, 'worldwide-clinic.php' );
$builder = f08pkg_file( $root, 'tools/build-candidate.php' );
$verifier = f08pkg_file( $root, 'tools/verify-candidate.php' );
$workflow = f08pkg_file( $root, '.github/workflows/file08-complete-quality.yml' );

f08pkg_has( 'runtime source', $plugin, 'Version: 1.2.0' );
f08pkg_has( 'runtime source', $plugin, "define( 'WCA_VERSION', '1.2.0' )" );
...
if ( $failures ) {
	fwrite( STDERR, "Release-package contract failed:\n- " . implode( "\n- ", $failures ) . "\n" );
	exit( 1 );
}
echo "Release-package runtime parity assertions passed: {$checks}/{$checks}.\n";
